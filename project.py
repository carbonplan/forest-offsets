...
class ARBProject(object):
    ...
    def _load_project_data(self):
        with open('data/projects.json') as f: # TODO some config magic to make fnames relative to repo, not my machine
            all_proj = json.load(f)
            proj_d = all_proj.get(self._proj_id)
            self._data = get_proj_series(proj_d)

    def _load_geometry(self):
        self._geom = gpd.read_file(f"data/geometry/projects/{self._proj_id}.json").to_crs(WORKING_CRS).iloc[0].geometry
